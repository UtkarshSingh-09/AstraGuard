...
### 7. POST `/api/upload-form16` — Form 16 PDF Upload (NEW 🔥)

**User uploads their Form 16 PDF → auto-extracts salary, deductions, HRA, 80C, 80D, home loan — fills 80% of Financial DNA automatically.**
...
async def upload_form16(user_id: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        # Step 3: Merge extracted data into user's financial_dna in MongoDB
        await db.users.update_one(
            {"_id": user_id},
            {"$set": {
                "financial_dna": result["financial_dna"],
                "form16_raw": result["raw_extracted"],
                "form16_metadata": result["metadata"],
            }},
            upsert=True,
        )

        # Step 4: Auto-run tax engine with extracted inputs
        tax_calc = tax_engine.calculate(**result["tax_inputs"])

        # Step 5: Get AI narration
        ...
        return {
            "extraction": result,      # what was extracted
            "tax_analysis": ai_result,  # AI narration of tax
        }
...
{
    "success": true,
    "financial_dna": {                          # ← merge this into MongoDB
        "annual_salary": 1800000,
        "monthly_expenses": 52500,              # estimated from take-home
        "existing_investments": {
            "epf": 108000,
            "nps": 50000,
            "ppf": 150000                       # 80C total attributed here
        },
        "has_home_loan": true,
        "home_loan_interest_annual": 185000,
        "hra_received": 360000
    },
    "tax_inputs": {                             # ← pass this to tax_engine.calculate()
        "annual_salary": 1800000,
        "hra_received": 360000,
        ...
        "deduction_80d": 25000,
        "home_loan_interest_24b": 185000,
        "professional_tax": 2400,
        "total_income": 1380000,
        "total_tax_paid": 161200,
        "epf_contribution": 108000
    },
    "metadata": {
        "pan": "ABCDE1234F",
    ...
    "fields_found": 14,
    "fields_missing": [],
    "extraction_method": "pdfplumber"           # or "llm_fallback"
}
```

**Fields extracted from Form 16:**

| Field | Source in Form 16 |
|---|---|
| `gross_salary` | Part B — Salary as per 17(1) |
| `hra_received` | Part B — Allowances u/s 10(13A) |
| `standard_deduction` | Part B — Standard Deduction |
| `deduction_80c` | Part B — Chapter VI-A (80C) |
| `deduction_80ccd_1b` | Part B — 80CCD(1B) NPS |
| `deduction_80d` | Part B — 80D Health Insurance |
| `home_loan_interest_24b` | Part B — Income from HP / 24(b) |
| `professional_tax` | Part B — Tax on Employment |
| `epf_contribution` | Part A or Part B — EPF |
| `total_income` | Part B — Gross Total Income |
| `total_tax` | Part B — Tax Payable |
| `pan` | Part A header |
| `assessment_year` | Part A header |
| `employer_name` | Part A header |

---

### 8. POST `/api/upload-cas` — CAMS/KFintech CAS Upload (NEW 🔥)

**User uploads their Mutual Fund CAS PDF (usually password-protected with PAN) → auto-extracts all active mutual funds, SIPs, and NAVs into the Portfolio Engine.**
...
async def upload_cas(
    user_id: str = Form(...), 
    password: str = Form(...), # Usually user's PAN in uppercase
    ...
    try:
        # Step 1: Parse CAS PDF using casparser
        result = await extract_from_cas(tmp_path, password)

        if not result["success"]:
            return {"error": result["error"]}

        # Step 2: Auto-run Portfolio Engine using extracted funds
        portfolio_calc = portfolio_engine.analyze(funds=result["funds"])

        # Step 3: Get AI narration for the portfolio
        ai_result = await run_orchestrator(
            user_id=user_id,
